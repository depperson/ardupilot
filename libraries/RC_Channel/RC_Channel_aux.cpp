...
RC_Channel_aux::output_ch(void)
{
    ...
    switch(function)
    {
        ...
        case k_manual:              // manual
            radio_out = radio_in;
        case k_invert_passthrough:  // invert
            radio_out = radio_in;   // TODO: add inversion logic 
        break;
    }
    ...
    for (uint8_t i = 0; i < RC_AUX_MAX_CHANNELS; i++) {
        ...
        if (ch && ch->function.get() == function) {
            ...
            if ((ch->function.get() == k_manual) || 
                (ch->function.get() == k_invert_passthrough )) {
